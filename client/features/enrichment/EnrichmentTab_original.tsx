...
const EnrichmentTab = () => {
    ...
    const groupedFields = useMemo(() => {
        return ALL_FIELDS.reduce((acc, field) => {
            ...
            return acc;
// Fix: Use Field[] for better type clarity.
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        {!file ? (
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center p-4 bg-slate-50 border border-slate-200 rounded-lg">
                                    ...
                                    <button onClick={() => { setFile(null); setValidationResult(null); }} className="text-slate-400 hover:text-red-500"><XCircle /></button>
                                </div>
                                {validationResult && (
                    ...
                    <Card>
                        ...
                        <div className="flex flex-wrap gap-2 mb-6">
                            {FIELD_PACKAGES.map(pkg => (
                                <button key={pkg.id} onClick={() => handlePackageSelect(pkg.id)} className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${activePackage === pkg.id ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                                    {pkg.name} - {pkg.total_credits} credits
                                </button>
                            ))}
                             <button onClick={() => setActivePackage('custom')} className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${activePackage === 'custom' ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                                Custom
                            </button>
                        </div>
                                    ...
                                    <div className="space-y-2">
                                        {fields.map(field => (
                                            <label key={field.id} className="flex items-center p-2 rounded-md hover:bg-slate-50 text-sm">
                                                <input type="checkbox" checked={selectedFields.includes(field.id)} onChange={() => handleFieldToggle(field.id)} className="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    <button 
                        disabled={!costEstimate || !costEstimate.can_afford} 
                    ...
                    >
                        🚀 Start Enrichment
                        {costEstimate && ` - Use ${costEstimate.total_credits.toLocaleString()} Credits`}
                    </button>
                </div>
            </div>
        </div>
